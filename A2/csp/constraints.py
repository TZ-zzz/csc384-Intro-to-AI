...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        for i_position in qi.domain():
            for j_position in qj.domain():
                if i_position != j_position and abs(i_position - j_position) != abs(i - j):
                    satisfying.append([i_position, j_position])
        TableConstraint.__init__(self, name, scope, satisfying)
...
class AllDiffConstraint(Constraint):
    """All diff constraint between a set of variables
       ...
       an AllDiff see
       https://www.constraint-programming.com/people/regin/papers/alldiff.pdf"""
    ...
    def hasSupport(self, var, val):
        ...
        def PartialNValue(l):
            ...
            for value in vals:
                if value in self._required:
                    count += 1
            num_unassigned = self.arity() - len(l)
            return num_unassigned + count >= self._lb and count <= self._ub

        varsToAssign = self.scope()
        varsToAssign.remove(var)
        x = findvals(varsToAssign, [(var, val)], NValue, PartialNValue)
        return x
