...
def sudokuCSP(initial_sudoku_board, model='neq'):
    ...
    for row in var_array:
        if model == 'neq':
            ...
            row_constraint = AllDiffConstraint("C_allDiff{}".format(row), row)
            constraint_list.extend([row_constraint])
    ...
    for colj in range(len(var_array[0])):
        ...
        if model == 'neq':
            ...
            col_constraint = AllDiffConstraint("C_allDiff{}".format(scope), scope)
            constraint_list.extend([col_constraint])
    ...
    for i in [0, 3, 6]:
        for j in [0, 3, 6]:
            ...
            if model == 'neq':
                ...
                square_constraint = AllDiffConstraint("C_allDiff{}".format(scope), scope)
                constraint_list.extend([square_constraint])
...
def solve_planes(planes_problem, algo, allsolns,
    ...
    m_f = planes_problem.maintenance_flights
    m = planes_problem.min_maintenance_frequency
    variables = []
    constraints = []
    flatten_variables = []

    # BUILD your CSP here and store it in the variable csp
    # for each plane, it can at most has a sequence of legal flight, we create a variable for each one
    for i in range(len(planes)):
        variables.append([])
        legal_flight = planes_problem.can_fly(planes[i])
        for j in range(len(legal_flight)):
            domain = []
            if j == 0:
                domain = planes_problem.can_start(planes[i])
            if j != 0:
                domain = legal_flight.copy()
            domain.extend(["End"])
            name = "V_{}.{}".format(planes[i], i)
            var = Variable(name, domain)
            variables[i].append(var)
            flatten_variables.append(var)
    # flight can only appear once and each flight should be different

    for flight in planes_problem.flights:
        constraints.append(NValuesConstraint("C{}".format(flight), flatten_variables, [flight], 1, 1))

    # the sequence of flights of a plane should be feasible
    for flight in planes_problem.flights:
        can_follow.append((flight, "End"))
    can_follow.append(("End", "End"))

    m_f.append("End")

    for i in range(len(variables)):
        # make one plane sequence feasible
        for j in range(len(variables[i]) - 1):
            # satisfyingAssignment = [follow for follow in can_follow if follow[0] in variables[i][j].domain()]
            constraints.append(TableConstraint("C_feasible {}{}".format(i, j), [variables[i][j], variables[i][j + 1]], can_follow))
        # ensure the maintenance
        for k in range(len(variables[i]) - m + 1):
            constraints.append(NValuesConstraint("C_maintain{}".format(i), variables[i][k:k + m], m_f, 1, m))
    ...
    for sol in solutions:
        sol_sequence = []
        for plane in planes:
            plane_route = [(var, val) for (var, val) in sol if var.name().split('.')[0] == "V_{}".format(plane)]
            # plane_route.sort(key=lambda x: x[0].name().split('.')[1])
            plane_route2 = [val for (var, val) in plane_route if val != "End"]
            plane_route2.insert(0, plane)
            sol_sequence.append(plane_route2)
        result.append(sol_sequence)
